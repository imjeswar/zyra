...
import zyra.echo.music.constants.ListenTogetherAutoApprovalKey
import zyra.echo.music.constants.ListenTogetherIsHostKey
import zyra.echo.music.constants.ListenTogetherRoomCodeKey
import zyra.echo.music.constants.ListenTogetherServerUrlKey
import zyra.echo.music.constants.ListenTogetherSessionTimestampKey
import zyra.echo.music.constants.ListenTogetherSessionTokenKey
import zyra.echo.music.constants.ListenTogetherUserIdKey
...
class ListenTogetherClient @Inject constructor(
    ...
    companion object {
        ...
        const val ACTION_APPROVE_JOIN = "zyra.echo.music.LISTEN_TOGETHER_APPROVE_JOIN"
        const val ACTION_REJECT_JOIN = "zyra.echo.music.LISTEN_TOGETHER_REJECT_JOIN"
        const val ACTION_APPROVE_SUGGESTION = "zyra.echo.music.LISTEN_TOGETHER_APPROVE_SUGGESTION"
        const val ACTION_REJECT_SUGGESTION = "zyra.echo.music.LISTEN_TOGETHER_REJECT_SUGGESTION"
    ...
    private fun loadBlockedUsernames() {
        try {
            val blockedJson = context.dataStore.get(zyra.echo.music.constants.ListenTogetherBlockedUsersKey, "")
    ...
    private suspend fun saveBlockedUsernames() {
        try {
            ...
            context.dataStore.edit { preferences ->
                preferences[zyra.echo.music.constants.ListenTogetherBlockedUsersKey] = blockedJson
            }
        } catch (e: Exception) {
    ...
    private inline fun <reified T> sendMessage(type: String, payload: T?) {
        try {
            ...
            val success = if (codec.format == MessageFormat.JSON) {
                webSocket?.send(data.decodeToString()) ?: false
            } else {
